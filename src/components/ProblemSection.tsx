...
export default function ProblemSection() {
  const cards = [
    {
      icon: <VolumeX className="h-6 w-6 text-blush-400" />,
    ...
    {
      icon: <Target className="h-6 w-6 text-sage-400" />,
    ...
    {
      icon: <Users className="h-6 w-6 text-mist-300" />,
    ...
    {
      icon: <Compass className="h-6 w-6 text-ink-500" />,
      ...
      bg: "bg-white ring-1 ring-ink-900/5"
    }
  ];
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 pb-10 lg:px-8">
      <motion.div 
        ...
        className="rounded-[2rem] border border-white/80 bg-white/70 px-6 py-8 shadow-card backdrop-blur md:px-8"
      >
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.26em] text-blush-400">Quando procurar ajuda</p>
            <h2 className="mt-3 font-display text-3xl text-ink-900 md:text-4xl">Você não precisa passar por isso sozinho.</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {cards.map((card, index) => (
              <motion.div 
                ...
                className={`rounded-3xl p-5 ${card.bg}`}
              >
                <div className="mb-4 inline-flex rounded-xl bg-white/50 p-2 backdrop-blur-sm">
                  {card.icon}
                </div>
